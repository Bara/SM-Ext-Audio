...
fn main() {
    ...
    for file in &["wepoll.c", "wepoll.h"] {
        std::fs::copy(src_dir.join(file), build_dir.join(file)).unwrap_or_else(
            |_| panic!("Failed to copy {} to the build directory", file),
        );
    }
    ...
    if build.try_compile("wepoll").is_ok() {
